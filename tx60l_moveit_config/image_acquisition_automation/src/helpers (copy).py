...
def create_points(planNum):
  xBound = np.arange(-0.05, 0.05, 0.02)
  yBound = np.arange(-0.02, 0.02, 0.02)
  zBound = np.arange(1, 1.2, 0.02)
...
def view3D(new_x, new_y, new_z, newtext):
  ...
  pointx = np.array((-0.02))
  pointy = np.array((-0.019))
  pointz = np.array((1.09))
